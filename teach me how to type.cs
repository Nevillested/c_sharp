...
namespace C_sharp
{
    ...
    class Program
    {
        public static int x = 1; //счетчик попыток
        public static double max = 0.0; //лучший результат
        public static double min = 0.0; //худший результат
        public static double avg = 0.0; //средний результат
        class result
        {


            public void getresult(ref int random, double dime, TimeSpan span)
            {
        ...
        public static int Minimum(int a, int b, int c) => (a = a < b ? a : b) < c ? a : c; //получение минимального из трех расстояния Левенштейна
        public static int LevenshteinDistance(string firstWord, string secondWord) //Итеративная реализация Левенштейна
        {
        ...
        static void Main(string[] args)
        {
            ...
            do
            {
                ...
                result popitka = new result();

                if (String.Compare(dict[keyy][random], stroka) == 0) //если ошибок нет, то...
                {
                    popitka.getresult(ref random, dime, span);
                }
